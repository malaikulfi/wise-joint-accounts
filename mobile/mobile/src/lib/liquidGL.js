...
(() => {
  ...
  class liquidGLRenderer {
    ...
    addLens(element, options) {
      ...
      if (maxZ > 0) {
        // Cap canvas z-index so it doesn't block interactive elements above
        const cappedZ = Math.min(maxZ - 1, 40);
        this.canvas.style.zIndex = cappedZ;
      }
  ...
  class liquidGLLens {
    constructor(renderer, element, options) {
      ...
      // Keep element visible so it stays interactive — WebGL renders behind it
    ...
    _reveal() {
      if (this.revealTypeIndex === 0) {
        // Keep element visible — don't touch opacity
