...
> **Responsive radius:** Border radius values may differ between mobile and desktop contexts. On smaller screens, components like cards and sheets often use reduced or zero radius (edge-to-edge), while the same components use `--radius-medium` or `--radius-large` on desktop.
...
|------|-----------|----------------|
| XS | 0px | Small phones |
| S | 480px | Large phones |
| M | 768px | Tablets |
| L | 992px | Small desktops |
| XL | 1200px | Desktops |
| XXL | 1440px | Large desktops |

### Grid Columns

- **Mobile (XS–S):** 6 columns
- **Tablet and above (M+):** 12 columns

### CSS Media Query Pattern

```css
/* Mobile-first: styles apply from 0px up */
.component { padding: var(--padding-small); }

/* Tablet */
@media (min-width: 768px) {
  .component { padding: var(--padding-medium); }
}

/* Desktop */
@media (min-width: 992px) {
  .component { padding: var(--padding-large); }
}
